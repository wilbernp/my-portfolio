...
export default function Contact() {
    return (
        <div className="mt-24 lg:bg-contact-image bg-no-repeat bg-right-bottom bg-[length:250px_500px]">
            {/* titulo */}
            <h2 className='text-center text-secondary text-3xl font-bold mb-14 sm:text-4xl' >CONTACTO</h2>
            {/* container */}
                ...
                <div className="w-full md:w-[70%] max-w-md lg:w-60 flex flex-col gap-8 lg:gap-10">
                    ...
                    <div className="flex flex-col gap-8">
                        ...
                        <a className='flex items-center gap-6 w-max' href="#">
                            <div className="bg-secondary flex items-center rounded-full w-10 h-10">
                            ...
                            contactLinksData.map((link, index) => {
                                return (
                                    <a href={link.href} key={index}>
                                        <InlineIcon icon={link.icon} className='text-secondary text-[2.8rem] transition-transform duration-500 hover:scale-125' />
                                    </a>
                                )
                            })
                        }
